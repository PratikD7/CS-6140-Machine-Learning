...
class decisionNode():
    ...
    def buildTree(self, sorted_data, min_size):
        ...
        for feature in range(0, count_of_features):
            ...
            a = temp_data[:,feature]
            attr_values = self.uniqueCounts(a)
            for i in attr_values.keys():
                set1, set2 = self.divideSet(sorted_data, feature, i)
...
def main():
    ...
    print("Confusion matrix for mushroom")
    ...
    plt.scatter(x1,y)
    plt.scatter(x2,y)
    plt.title("Accuracy comparison for MUSHROOM-binary")
    plt.xlabel("Accuracy")
    plt.ylabel("eta_min")
