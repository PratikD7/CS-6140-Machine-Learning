...
def M_fold_CV(train_data, test_data, Y_train, Y_test, n_iterations, tolerance, learning_rate,
              flag, C, m_folds):
    accuracy_score_matrix = [0.0 for i in range(len(C))]
    ...
    for fold in folds:
        ...
        for c_value in C:
            ...
            accuracy_score_matrix[idx] += score
            idx += 1

    return C[accuracy_score_matrix.index(max(accuracy_score_matrix))]


# =============================================================================#
# SVM algorithm call
def SVM_algorithm(ds, k_folds, n_iterations, tolerance, learning_rate, flag, C, m_folds):
    # Split the dataset into training and cross validation set
    ...
    for dataset in datasets_list:
        ...
        for i in range(3):
            ...
            accuracy_train, precision_train, recall_train, accuracy_test, precision_test, recall_test, c_value \
                = SVM_algorithm(ds, k_folds, n_iterations, tolerance, learning_rate, flag, C, m_folds)
            flag+=1
