...
def M_fold_CV(train_data, test_data, Y_train, Y_test, n_iterations, tolerance, learning_rate,
    ...
    accuracy_score_matrix = [[0.0 for i in range(len(C))] for j in range(len(gamma))]
    ...
    for fold in folds:
        ...
        for g_value in gamma:
            ...
            for c_value in C:
                ...
                accuracy_score_matrix[idx_i][idx_j] += score
                idx_j += 1
            idx_i += 1

    accuracy_score_matrix = numpy.array(accuracy_score_matrix)
    indices = numpy.unravel_index(accuracy_score_matrix.argmax(), accuracy_score_matrix.shape)
