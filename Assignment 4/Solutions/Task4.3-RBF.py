...
def M_fold_CV(train_data, test_data, Y_train, Y_test, n_iterations, tolerance, learning_rate,
    ...
    auc_score_matrix = [[0.0 for i in range(len(C))] for j in range(len(gamma))]
    ...
    for fold in folds:
        ...
        for g_value in gamma:
            ...
            idx_j = 0
            for c_value in C:
                model = svm.SVC(kernel='rbf', C=c_value, gamma=g_value, probability=True)
                ...
                roc_auc = auc(fpr, tpr)
                auc_score_matrix[idx_i][idx_j] += roc_auc
                idx_j += 1
            idx_i += 1
        print("-"*50)
...
# =============================================================================#
# SVM algorithm call
def SVM_algorithm(train_data, test_data, k_folds, n_iterations, tolerance, learning_rate, flag, gamma, C, m_folds):
...
def main():
    ...
    for i in range(10):
        ...
        accuracy_train, precision_train, recall_train, accuracy_test, precision_test, recall_test, c_value, g_value \
            = SVM_algorithm(train_data, test_data, k_folds, n_iterations, tolerance, learning_rate, flag, gamma, C,
                            m_folds)
        flag += 1
        print("Best values of C: ", end='')
        print(c_value)
        print("Best values of Gamma: ", end='')
        print(g_value)
