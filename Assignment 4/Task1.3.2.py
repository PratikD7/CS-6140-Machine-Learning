...
def gradient_descent(train_data, Y_train, n_iterations, gamma):
...
def dual_perceptron_with_gaussian_kernel(train_data, test_data, Y_train, Y_test, n_iterations, tolerance, learning_rate,
                                         flag, gamma):
    preds_train = []
    preds_test = []
    # Calculating the coefficients from the Gradient Descent algorithm
    coeff, train_data, alpha = gradient_descent(train_data, Y_train, n_iterations, gamma)
    print("-" * 50)
    # final_scores = np.dot(train_data, coeff)
    for row in train_data:
        preds_train.append(predict(row, train_data, alpha, Y_train, gamma))
    ...
    for row in test_data:
        preds_test.append(predict(row, train_data, alpha, Y_train, gamma))
...
def dual_perceptron_algorithm(ds, n_folds, n_iterations, tolerance, learning_rate, flag, gamma):
    # Split the dataset into training and cross validation set
    ...
    for fold in folds:
        ...
        accuracy_train, precision_train, recall_train, accuracy_test, precision_test, recall_test = \
            dual_perceptron_with_gaussian_kernel(train_data, test_data, Y_train, Y_test, n_iterations, tolerance,
                                                 learning_rate, flag, gamma)
...
def main():
    ...
    for gamma in gamma_values:
        for dataset in datasets_list:
            ...
            accuracy_train, precision_train, recall_train, accuracy_test, precision_test, recall_test \
                = dual_perceptron_algorithm(ds, n_folds, n_iterations, tolerance, learning_rate, flag, gamma)
