...
def gradient_descent(train_data, Y_train, learning_rate, n_iterations, tolerance, flag, gamma):
    ...
    for step in range(n_iterations):
       ...
       for index in range(len(train_data)):
           ...
           SVM_loss += np.log(1+np.exp(wtxi)) - Y_train[index]*wtxi
       ll.append(SVM_loss)
       '''
...
# =============================================================================#
# SVM regression algorithm implementation
def M_fold_CV(train_data, test_data, Y_train, Y_test, n_iterations, tolerance, learning_rate,
    ...
    for fold in folds:
        ...
        for c_value in C:
            ...
            fpr, tpr, thresholds = roc_curve(inner_Y_test, probability_predictions)
            roc_auc = auc(fpr, tpr)
...
def SVM_algorithm(ds, k_folds, n_iterations, tolerance, learning_rate, flag, gamma, C, m_folds):
    ...
    for fold in folds:
        ...
        # SVM Regression algorithm function call
        ...
        plt.plot(fpr, tpr)
        # plt.title("Breast Cancer: ROC Curve for fold number ", index+1)
...
def main():
    ...
    for dataset in datasets_list:
        ...
        # Parameters for the SVM regression gradient descent algorithm
        k_folds = 10
        m_folds = 5
